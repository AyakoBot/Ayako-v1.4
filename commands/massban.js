...
module.exports = {
	...
	async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID ) {
		/* eslint-enable */
		if (args[0] == 'ids') {
			delete require.cache[require.resolve('../ids.json')];
			args = require('../ids.json').ids;
		}
							...
							if (user.id !== msg.author.id) {
								if (msg.guild.member(user)) {
									if (+msg.guild.member(msg.author).roles.highest.rawPosition > +msg.guild.member(user).roles.highest.rawPosition) {
										if (+msg.guild.member(msg.author).roles.highest.rawPosition !== +msg.guild.member(user).roles.highest.rawPosition) {
											if (msg.guild.member(user).bannable) users.push(user);
											else failed.push(`${arg} (unable to ban)`);
										} else failed.push(`${arg} (You cant ban this user)`);
									} else failed.push(`${arg} (You cant ban this user)`);
								} else users.push(user);
							} else failed.push(`${arg} (You cant ban yourself)`);
						}
		...
		if (uniqueUsers.length !== 0) {
			const replyEmbed = new Discord.MessageEmbed()
				...
				.setAuthor('Successfully banned | Loading', 'https://cdn.discordapp.com/emojis/670163928122130444.gif?v=1', 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands')
				.setTimestamp();
			...
			const intervalS = setInterval(async () => {
				if (descS.length == uniqueUsers.length) {
					...
					replyEmbed.setAuthor('Finished bans', 'https://cdn.discordapp.com/emojis/902269933860290650.png?v=1', 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands');
		...
		if (uniqueFails.length !== 0) {
			const replyEmbed = new Discord.MessageEmbed()
				...
				.setAuthor('Failed to ban | Loading', 'https://cdn.discordapp.com/emojis/670163928122130444.gif?v=1', 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands')
				.setTimestamp();
			const m = await msg.channel.send(replyEmbed);
			...
			const editintervalF = setInterval(async () => {
				if (descF.length !== uniqueFails.length) await m.edit(replyEmbed);
			}, 5000);
			const intervalF = setInterval(async () => {
				if (descF.length == uniqueFails.length) {
					...
					replyEmbed.setAuthor('Failed to ban', 'https://cdn.discordapp.com/emojis/902269073805701211.png?v=1', 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands');
