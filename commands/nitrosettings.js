...
module.exports = {
  ...
  async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID) {
    ...
    if (resS.rowCount == 0)
      return msg
        .reply(
          'There are no settings I can display, start by setting up your Nitro Boosters with `h!nitrosetup`',
        )
        .catch(() => {});
    ...
    const resI = await pool.query(
      `SELECT * FROM nitroboosters WHERE guildid = '${msg.guild.id}' AND stillactive = 'false'`,
    );
    const resA = await pool.query(
      `SELECT * FROM nitroboosters WHERE guildid = '${msg.guild.id}' AND stillactive = 'true'`,
    );
    ...
    embed.addFields(
      ...
      {
        name: '\u200b',
        value: 'Number of boosters not correct?\n Wait till 2-3 am GMT',
        inline: false,
      },
    );
    embed.setColor('b0ff00');
	  embed.addField(
		  '⚠️Notice!',
		  'This Feature is not currently working reliably!\nThere will be an update very soon to fix this and restore previous Boosters.\nPlease bare with me :)',
	  );
    embed.setAuthor(
      'Ayako Nitro Monitor [Click here to invite me]',
    ...
    async function rolechecker(roleid) {
      ...
      if (!role || role == undefined) {
        pool.query(
          `DELETE FROM nitroroles WHERE roleid = '${roleid}' AND guildid = '${msg.guild.id}'`,
        );
        return true;
