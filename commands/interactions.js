...
module.exports = {
	...
	async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID ) {
		...
		async function interaction2Function(command1, continued, msg) {
			if ((command1 == 'lick' || command1 == 'bloodsuck' || command1 == 'nom') && msg.guild.id == '108176345204264960') return;
			let CD;
			...
			if (command1 === 'hug' || command1 === 'comfort') {
				var random = Math.round(Math.random() * 42);
				let gif = '';
			...
			if (command1 === 'kiss' || command1 === 'kith' || command1 === 'smolkith' || command1 === 'smallkiss') {
				...
				if (random == 32) {gif = 'https://cdn.discordapp.com/attachments/760152457799401532/806682035326156800/tenor_44.gif';}
				processedInteraction2Function(Text, gif, CD);
			}
			if (command1 === 'lick') {
			...
			if (command1 === 'cuddle' || command1 === 'nuzzle' || command1 === 'snuggle') {
				random = Math.round(Math.random() * 28);
				let gif = '';
			...
			if (command1 === 'pat') {
				random = Math.round(Math.random() * 49);
				let gif = '';
