...
/* eslint-disable */
  async execute(msg, args, client, prefix, auth, command, logchannelid, permLevel, errorchannelID) {
    /* eslint-enable */
		let enabled = '<:Cross:902269073805701211>';
		let role = '<:Cross:902269073805701211>';
		const res = await pool.query(`SELECT * FROM disboard WHERE guildid = '${msg.guild.id}'`);
		if (res) {
			if (res.rows[0]) {
				if (res.rows[0].enabled == true) {enabled = '<:Tick:902269933860290650>';}
		...
		if (!args[0]) {
			...
			const embed = new Discord.MessageEmbed()
				.setAuthor('Ayako DISBOARD Bump Reminder Settings', client.user.displayAvatarURL(), 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands')
				.setDescription(`
