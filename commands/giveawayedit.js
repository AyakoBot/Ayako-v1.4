...
module.exports = {
	...
	async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID) {
		...
		const embed = new Discord.MessageEmbed()
			.setColor('b0ff00')
			.setAuthor('Edit a Giveaway', 'https://www.ayakobot.com', 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands')
		...
		async function Editing() {
			...
			if (editing == 'endtime') {
				...
				if (!isNaN(answer)) {
					...
					if (server && server.id) {
						const m = await Send(`Trying to create an Invite for ${server.name} <a:Loading:780543908861182013>`);						
						...
						let invite;
						if (inv && inv.url) {
							m.edit(`<:Tick:902269933860290650> Successfully created an Invite -> ${inv.url}`);
							invite = inv.url;
							pool.query(`
							...
							m.edit('<:Cross:902269073805701211> I wasnt able to create an Invite, you can enter a custom invite. Reply with the invite url or `no` which will abort the Giveaway editing process');
