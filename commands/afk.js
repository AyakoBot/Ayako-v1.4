...
/* eslint-disable */
  async execute(msg, args, client, prefix, auth, command, logchannelid, permLevel, errorchannelID) {
    /* eslint-enable */
	  ...
	  if (msg.author.id == '329328217838059521' && args[0]) text = `${args.slice(0).join(' ').replace(/'/g, '').replace(/`/g, '')}\n${msg.author.username.replace(/'/g, '')} has been summoned, but is AFK **since**`;
	  else if (msg.author.id == '329328217838059521')`${msg.author.username.replace(/'/g, '')} has been summoned, but is AFK **since**`;
	  else if (args[0]) text = `${args.slice(0).join(' ').replace(/'/g, '').replace(/`/g, '')}\n${msg.author.username.replace(/'/g, '')} is AFK **since**`;
