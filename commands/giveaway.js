...
module.exports = {
  ...
  execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        'What channel do you want the giveaway to be in? Reply with a **Channel Mention** or **Channel ID**',
      )
    ...
    function Channel(msg) {
      msg.channel
        ...
        .then((collected) => {
          ...
          if (!channel) {
            msg.reply(
              'That channel doesnt exist or i cant view it, please update the settings or enter a valid channel',
            );
            Channel(msg);
            return;
          }
    ...
    function Winners(msg, channel, prize) {
      msg.channel
        ...
        .then((collected) => {
          ...
          if (isNaN(winners)) {
            msg.reply(
              'The Number you entered is not actually a Number, reply with an actual Number please',
            );
            Winners(msg, channel, prize);
            return;
          }
    ...
    function Time(msg, channel, prize, winners) {
      msg.channel
        ...
        .then((collected) => {
          ...
          if (isNaN(time)) {
            msg.reply(
              'Whatever you entered was not a valid time, please follow this pattern: `Days/D Hours/H Minutes/M` Example: 7 days',
            );
            Time(msg, channel, prize, winners);
            return;
          }
      ...
      msg.channel
        ...
        .then((collected) => {
          ...
          if (req == 'no') {
            ...
            if (!req.includes('&')) {
              msg.reply(
                'That was not a valid Role ID / Role Mention / Server ID | Please enter a correct one now.',
              );
              Req(msg, channel, prize, winners, time);
              return;
            } else {
            ...
            if (isNaN(req)) {
              ...
              if (!role || roletext == 'undefined') {
                ...
                if (!server || guildtext == 'undefined') {
                  msg.reply(
                    "Since your Server does not have any roles with matching ID I guess that was a Server ID. Sadly I can only apply Server requirements if I'm in that server since I need to access their Member List\n Use `h!invite` to view my Invite and invite me there, then start over.",
                  );
                  return;
                } else {
                  requirements = 'guild';
                  ...
                  systemchannel
                    .createInvite({
                      maxAge: Math.floor(time / 1000 + 5),
                      reason: `Giveaway on ${msg.guild.name}`,
                    })
                    .then((invite) => {
                    ...
                    .catch(() => {});
                  msg.channel.send(
                    `Trying to create an Invite for ${server.name} <a:Loading:780543908861182013>`,
                  );
                  setTimeout(() => {
                    if (inviteURL !== '') {
                      msg.channel.send(
                        `<:Tick:902269933860290650> Successfully created an Invite -> ${inviteURL}`,
                      );
                      finish(msg, channel, prize, winners, time, requirements, server, inviteURL);
                    } else {
                      msg.channel.send(
                        '<:Cross:902269073805701211> I wasnt able to create an Invite, you can enter a custom invite. Reply with `invite url` or `no` which will abort the giveaway process',
                      );
            ...
            msg.reply(
              'You didnt enter a Valid option, please reply with one of these options: `Role ID or Mention` `Server ID` or `No` for none',
            );
            Req(msg, channel, prize, winners, time);
            return;
          }
      ...
      msg.channel
        ...
        .then((collected) => {
          ...
          if (answer.includes('https://discord.gg/')) {
            ...
            prize += `\n\n**Requirement:**\nYou should be member of [${server.name}](${
              collected.first().content
            } "Join ${server.name} to claim the prize")`;
            finish(msg, channel, prize, winners, time, requirements, server, inviteURL);
          } else if (answer == 'no') {
            msg.reply(
              'Giveaway aborted, I only need "Create Invite" permissions in the System Channel. Thats all.',
            );
            return;
          } else {
    ...
    async function finish(msg, channel, prize, winners, time, requirements, server, inviteURL) {
      ...
      const giveawaymsg = await channel.send(testEmbed).catch((e) => {
        return msg.reply(
          'I wasnt able to send the embed, be sure i have `Send Messages` `Embed Links` and `Attach Files` permissions in that channel and start over\n' +
            e,
        );
      });
      time = +time + Date.now();
      const timeleft = String(time).slice(0, -3);
      let description;
      ...
      const giveawayEmbed = new Discord.MessageEmbed()
        ...
        .addField('React with 🎉 to participate!', `Ends <t:${timeleft}:R> (<t:${timeleft}:f>)`)
      ...
      if (requirements) {
        ...
        if (requirements == 'role') {
          ...
          pool.query(`INSERT INTO giveawaysettings( 
						...
						'${msg.guild.id}', '${channel.id}', '${giveawaymsg.id}', '${reqs}', '${role.id}', '${prize.replace(
            /'/g,
            '',
          )}', '${winners}', '${time}', 'false'
					)`);
        ...
        if (requirements == 'guild') {
          ...
          pool.query(`INSERT INTO giveawaysettings( 
						...
						'${msg.guild.id}', '${channel.id}', '${giveawaymsg.id}', '${reqs}', '${
            server.id
          }', '${inviteURL}', '${prize.replace(/'/g, '')}', '${winners}', '${time}', 'false'
					)`);
        }
      }
      if (!requirements) {
        pool.query(
          `INSERT INTO giveawaysettings(guildid, channelid, messageid, description, winnercount, endat, ended) VALUES ('${
            msg.guild.id
          }', '${channel.id}', '${giveawaymsg.id}', '${prize.replace(
            /'/g,
            '',
          )}', '${winners}', '${time}', 'false')`,
        );
