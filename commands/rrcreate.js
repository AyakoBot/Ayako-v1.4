...
module.exports = {
    ...
    /* eslint-enable */
		...
		const embed = new Discord.MessageEmbed()
			.setAuthor('Ayako Reaction Role Setup', client.user.displayAvatarURL(), 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands')
		...
		async function awaitRoles() {
			...
			if (collected.toLowerCase() == 'done') {
				...
				if (!role || !role.id) {
					rawcollected.first().react('902269073805701211').catch(() => {});
					awaitRoles();
					return;
				} else {
					...
					rawcollected.first().react('902269933860290650').catch(() => {});
		...
		async function done() {  
			const ReactionEmbed = new Discord.MessageEmbed()
				...
				.setColor('b0ff00')
				.setAuthor('Ayako Reaction Roles', client.user.displayAvatarURL(), 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands');
