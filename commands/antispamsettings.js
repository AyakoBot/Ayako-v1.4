...
module.exports = {
	...
	/* eslint-enable */
		...
		async function Continue(settings, content) {
			...
			if (settings.antispamToF) {antispam = '<:Tick:902269933860290650> Enabled';} else {antispam = '<:Cross:902269073805701211> Disabled';}
			if (settings.giveofficialWarnsToF) {give_official_warnigs_tof = '<:Tick:902269933860290650> Enabled';} else {give_official_warnigs_tof = '<:Cross:902269073805701211> Disabled';}
			if (settings.muteAfterWarnsAmount) {mute_after_warns_amount = settings.muteAfterWarnsAmount;} else {mute_after_warns_amount = '<:Cross:902269073805701211> Not Set';}
			if (settings.KickAfterWarnsAmount) {kick_after_warns_amount = settings.KickAfterWarnsAmount;} else {kick_after_warns_amount = '<:Cross:902269073805701211> Not Set';}
			if (settings.BanAfterWarnsAmount) {ban_after_warns_amount = settings.BanAfterWarnsAmount;} else {ban_after_warns_amount = '<:Cross:902269073805701211> Not Set';}
			if (settings.ofWarnEnabeldToF) {of_warn_enabeld_tof = '<:Tick:902269933860290650> Enabled';} else {of_warn_enabeld_tof = '<:Cross:902269073805701211> Disabled';}
			if (settings.muteEnabledToF) {mute_enabled_tof = '<:Tick:902269933860290650> Enabled';} else {mute_enabled_tof = '<:Cross:902269073805701211> Disabled';}
			if (settings.kickEnabledToF) {kick_enabled_tof = '<:Tick:902269933860290650> Enabled';} else {kick_enabled_tof = '<:Cross:902269073805701211> Disabled';}
			if (settings.banEnabledToF) {ban_enabled_tof = '<:Tick:902269933860290650> Enabled';} else {ban_enabled_tof = '<:Cross:902269073805701211> Disabled';}
			if (content) {
			...
			if (settings) {
				...
				embed.setColor('b0ff00');
				embed.setAuthor('Ayako Antispam [Click here to invite me]', client.user.displayAvatarURL(), 'https://discord.com/api/oauth2/authorize?client_id=650691698409734151&permissions=1642787630327&scope=bot%20applications.commands');
