...
module.exports = {
	...
	async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID ) {
		/* eslint-enable */
		if (!args[0]) return msg.reply('Do you want to `set` or `delete` a reminder? Or enter `advanced` reminder settings\n`h!reminder set [duration] [text]`\n`h!reminder delete [reminder ID]`\n`h!reminder advanced`');
		...
		if (option == 'set') {
			...
			await pool.query(`INSERT INTO reminders (text, duration, channelid, userid) VALUES ('${remindertext.replace(/'/g, '').replace(/`/g, '')}', '${Date.now() + duration}', '${msg.channel.id}', '${msg.author.id}');`);
			...
			if (res !== undefined && res.rows[0] !== undefined) {
				for (let i = 0; i < res.rowCount; i++) {
					...
					pool.query(`UPDATE reminders SET rnr = '${i}' WHERE channelid = '${channelid}' AND duration = '${duration}' AND userid = '${userid}' AND text = '${text.replace(/'/g, '').replace(/`/g, '')}';`);
				}
			}
			...
			const embed = new Discord.MessageEmbed()
				.setAuthor('Ayako Advanced Reminders', client.user.displayAvatarURL())
				.setColor('b0ff00')
				.setDescription('Respond with:\n`create` if you want to create a new Reminder\n`edit` if you want to edit an existing Reminder\n`delete` if you want to delete an existing Reminder')
				.addField('`list` if you want to list all your existing Reminders', '`list server` if you want to list existing Reminders in this Server\n`list (channel ID or mention)` if you want to list all Reminders in the given Channel\n`list (user ID or mention)` if you want to list all public Reminders of the given User');
			msg.channel.send(embed).catch(() => {});
			require('./reminders_advanced/split.js').exe(msg);
		} else {
