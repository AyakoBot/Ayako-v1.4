...
module.exports = {
	...
	async execute(msg, args, client, prefix, auth, command, logchannelid, permLevel, errorchannelID) {
		...
		cmds.Moderation.state = '<:Tick:902269933860290650> Enabled';
		cmds.ModerationAdvanced.state = '<:Tick:902269933860290650> Enabled';
		cmds.Fun.state = '<:Tick:902269933860290650> Enabled';
		cmds.Selfroles.state = '<:Tick:902269933860290650> Enabled';
		cmds.Suggestion.state = '<:Tick:902269933860290650> Enabled';
		cmds.Antispam.state = '<:Tick:902269933860290650> Enabled';
		cmds.Giveaway.state = '<:Tick:902269933860290650> Enabled';
		cmds.Welcome.state = '<:Tick:902269933860290650> Enabled';
		cmds.Nitro.state = '<:Tick:902269933860290650> Enabled';
		cmds.Leveling.state = '<:Tick:902269933860290650> Enabled';
		cmds.Blacklist.state = '<:Tick:902269933860290650> Enabled';
		cmds.Info.state = '<:Tick:902269933860290650> Enabled';
		cmds.Miscellaneous.state = '<:Tick:902269933860290650> Enabled';
		if (res) {
			if (res.rows[0]) {
				const r = res.rows[0];
				if (r.moderation == false)  {cmds.Moderation.state = '<:Cross:902269073805701211> Disabled';}
				if (r.moderationadvanced == false)  {cmds.ModerationAdvanced.state = '<:Cross:902269073805701211> Disabled';}
				if (r.fun == false)  {cmds.Fun.state = '<:Cross:902269073805701211> Disabled';}
				if (r.selfroles == false)  {cmds.Selfroles.state = '<:Cross:902269073805701211> Disabled';}
				if (r.suggestion == false)  {cmds.Suggestion.state = '<:Cross:902269073805701211> Disabled';}
				if (r.antispam == false)  {cmds.Antispam.state = '<:Cross:902269073805701211> Disabled';}
				if (r.giveaway == false)  {cmds.Giveaway.state = '<:Cross:902269073805701211> Disabled';}
				if (r.welcome == false)  {cmds.Welcome.state = '<:Cross:902269073805701211> Disabled';}
				if (r.nitro == false)  {cmds.Nitro.state = '<:Cross:902269073805701211> Disabled';}
				if (r.leveling == false)  {cmds.Leveling.state = '<:Cross:902269073805701211> Disabled';}
				if (r.blacklist == false)  {cmds.Blacklist.state = '<:Cross:902269073805701211> Disabled';}
				if (r.info == false)  {cmds.Info.state = '<:Cross:902269073805701211> Disabled';}
				if (r.miscellaneous == false)  {cmds.Miscellaneous.state = '<:Cross:902269073805701211> Disabled';}
			}
		}
