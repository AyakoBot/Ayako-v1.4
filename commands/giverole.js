...
module.exports = {
	...
	usage: 'h!giverole [user ID or mention] [Role ID, mention or name]',
	/* eslint-disable */
	execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID ) {		/* eslint-enable */
		if (!msg.guild.member(client.user).permissions.has('MANAGE_ROLES')) return msg.reply('I am not able to give roles to this user. Be sure I have the "Manage Roles" permission.');
		...
		async function muteFunction(user, logchannelid){
			if (!args[1]) msg.reply('Please mention the role or send the role ID to the role I should add to them -> `h!giverole [user ID or mention] [role ID or mention]`');
			...
			if (rawRole.includes('<@&') && rawRole.includes('>')) {
				...
				role = msg.guild.roles.cache.find(role => role.name.toLowerCase() === args.slice(1).join(' ').toLowerCase());
			}
			if (!role) {
				return msg.reply('That was not a valid role, please try again -> `h!giverole [user ID or mention] [role ID or mention]`');
			}
			if (msg.guild.member(msg.author).roles.highest.rawPosition < role.rawPosition || msg.guild.member(msg.author).roles.highest.rawPosition == role.rawPosition) {
				...
				if (Muterole && Muterole == role) {
					msg.reply('You cant mute members this way, please use the Mute commands.');
					return;
				}
				msg.guild.member(user).roles.add(role);
				const logchannel = client.channels.cache.get(logchannelid);
				const log = new Discord.MessageEmbed()
					.setTitle(user.username + ' was given the role '+ role.name)
					.setColor('YELLOW')
					.setThumbnail(user.displayAvatarURL())
					.setDescription(`${user} was given the ${role} role by ${msg.author}`)
					.setTimestamp()
