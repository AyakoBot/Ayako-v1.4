...
module.exports = {
	...
	description: 'Display the permissions of the current TrialModRole',
	usage: 'h!trialmodperms',
	/* eslint-disable */
	async execute(msg, args, client, prefix, command, logchannelid, permLevel, errorchannelID ) {
		...
		if (res && res.rowCount > 0 && res.rows[0].trialmodrole) {
			const role = res.rows[0].trialmodrole;
			const res2 = await pool.query(`SELECT * FROM modperms WHERE guildid = '${msg.guild.id}' AND type = 'trial'`);
			...
			const Embed = new Discord.MessageEmbed()
				.setTitle('TrialModRole permissions')
				.setColor('#b0ff00')
			...
			msg.reply('You have to set a TrialModRole before you can use this command.');
		}
	}
};